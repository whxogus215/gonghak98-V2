...
class GeneralMajorTest {
    ...
    private static final Set<String> GENERAL_COURSES = Set.of(
    ...
    void 일반영역_교과목_24학점_이상_이수(List<CompletedCourse> studentCourses) {
        //given
        int minPoint = 24;
        GeneralMajor generalMajor = new GeneralMajor(GENERAL_COURSES, minPoint);
    ...
    private static Stream<Arguments> providePassingGeneralCourseCombinations() {
        ...
        List<CompletedCourse> essential = List.of(
            CompletedCourse.builder().name("전기회로").point(3).build(),
            CompletedCourse.builder().name("물리전자공학").point(3).build(),
            CompletedCourse.builder().name("신호및시스템").point(3).build(),
            CompletedCourse.builder().name("기초설계").point(3).build()
        );

        List<CompletedCourse> elective1 = List.of(
            CompletedCourse.builder().name("통신이론").point(3).build(),
            CompletedCourse.builder().name("데이터통신").point(3).build(),
            CompletedCourse.builder().name("컴퓨터네트워크").point(3).build(),
            CompletedCourse.builder().name("영상처리").point(3).build()
        );
        List<CompletedCourse> elective2 = List.of(
            CompletedCourse.builder().name("전자회로1").point(3).build(),
            CompletedCourse.builder().name("전자기1").point(3).build(),
            CompletedCourse.builder().name("광전자공학").point(3).build(),
            CompletedCourse.builder().name("디지털통신시스템").point(3).build()
        );
